...
data <- read.csv("Data Sheet_Fome_AT2.csv")
...
model.W1.3 <- drm(biomass ~ rate, pop, fct = W1.3(),  data = data, type = "continuous")
...
ED50.AT2 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[3, 1] * 0.5), clevel = "AT12", type = "absolute", interval = "delta")

ED50.AT8.EDGE <- ED(model.W1.3, (summary(model.W1.3)$coefficients[4, 1] * 0.5), clevel = "AT15", type = "absolute", interval = "delta")


ED50.AT2 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[3, 1] * 0.1), clevel = "AT12", type = "absolute", interval = "delta")

ED50.AT8.EDGE <- ED(model.W1.3, (summary(model.W1.3)$coefficients[4, 1] * 0.1), clevel = "AT15", type = "absolute", interval = "delta")
...
EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[3, 1] * 0.5),
                     (summary(model.W1.3)$coefficients[4, 1] * 0.5)), 
       compMatch = c("AT12", "AT15"), type = "absolute", interval = "none", reverse = F)



EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[3, 1] * 0.1),
                     (summary(model.W1.3)$coefficients[4, 1] * 0.1)), 
       compMatch = c("AT12", "AT15"), type = "absolute", interval = "none", reverse = F)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Giant Ragweed/AT12/Fomesafen/NCWSS_2023/AT2_Fome_DR Biomass.tiff",
...
points(x = c(ED50.AT2[1, 1], 
             ED50.AT8.EDGE[1, 1]),
       y = c((summary(model.W1.3)$coefficients[3, 1]/2), 
             (summary(model.W1.3)$coefficients[4, 1]/2)),
       pch = 8,
       cex = 1.3)
