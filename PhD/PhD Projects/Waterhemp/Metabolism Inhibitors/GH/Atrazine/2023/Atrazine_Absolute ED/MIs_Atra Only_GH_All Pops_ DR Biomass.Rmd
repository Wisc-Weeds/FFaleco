...
data <- read.csv("Data Sheet_Atrazine_GH.csv")
...
summary.atra <- summaryBy(cbind(biomass) ~ pop + rate, data = subset(data_atra),
          FUN=fun)
...
model.LL4 <- drm(biomass ~ rate, pop, fct = LL.4(), data = data_atra, type = "continuous")
...
model.W2.3 <- drm(biomass ~ rate, pop, fct = W2.3(),  data = data_atra, type = "continuous")
...
# If the model was a LL we should use logBase = exp(1). Eg: EDcomp(model.W2.3, c((summary(model.W2.3)$coefficients[6, 1] * 0.5), (summary(model.W2.3)$coefficients[5, 1] * 0.5)), compMatch = c("A101", "A82"), type = "absolute", interval = "none", reverse = F, logBase = exp(1))
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Atrazine/2023/Atrazine_Absolute ED/MIs_Atra Only_GH_All Pops_ DR Biomass.tiff",
