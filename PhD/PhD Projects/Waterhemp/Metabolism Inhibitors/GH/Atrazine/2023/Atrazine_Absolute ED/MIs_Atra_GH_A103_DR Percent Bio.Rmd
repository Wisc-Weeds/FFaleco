...
summary.atra <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A103, herb=="atra"),
          FUN=fun)


summary.atra_GST <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A103, herb=="atra+GST"),
          FUN=fun)


summary.atra_P450 <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A103, herb=="atra+P450"),
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, herb, fct = LL.4(), data = data_A103, type = "continuous")
...
model.LL.3 <- drm(percent_bio ~ rate, herb, fct = LL.3(), data = data_A103, type = "continuous")
...
ED50.atra <- ED(model.LL.3, (summary(model.LL.3)$coefficients[4, 1] * 0.5), clevel = "atra", type = "absolute", interval = "delta")

ED50.atra.P450 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[5, 1] * 0.5), clevel = "atra+P450", type = "absolute", interval = "delta")

ED50.atra.GST <- ED(model.LL.3, (summary(model.LL.3)$coefficients[6, 1] * 0.5), clevel = "atra+GST", type = "absolute", interval = "delta")
...
EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[4, 1] * 0.5),
       ...
       compMatch = c("atra", "atra+P450"), type = "absolute", interval = "none", reverse = T)


EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[4, 1] * 0.5),
                     (summary(model.LL.3)$coefficients[6, 1] * 0.5)), 
       compMatch = c("atra", "atra+GST"), type = "absolute", interval = "none", reverse = T)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Atrazine/2023/Atrazine_Absolute ED/MIs_Atra_GH_A103_DR Percent Bio.tiff",
...
plot(model.LL.3,
     ...
     ylab = expression(paste("A103 Biomass (%)")),
     ylim = c(0, 100),
     #broken = T,
