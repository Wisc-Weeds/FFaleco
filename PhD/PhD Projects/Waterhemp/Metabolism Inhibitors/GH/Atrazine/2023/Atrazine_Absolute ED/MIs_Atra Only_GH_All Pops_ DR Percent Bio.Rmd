---
title: "Waterhemp MIs GH Atrazine Percent Bio"
...
data <- read.csv("Data Sheet_Atrazine_GH_2023.csv")

str(data)


data_atra <- data %>% filter(herb == "atra")
...
summary.atra <- summaryBy(cbind(percent_bio) ~ pop + rate, data = subset(data_atra),
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, pop, fct = LL.4(), data = data_atra, type = "continuous")
...
model.LL.3 <- drm(percent_bio ~ rate, pop, fct = LL.3(), data = data_atra, type = "continuous")
...
ED50.A82 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[5, 1] * 0.5), clevel = "A82", type = "absolute", interval = "delta")

ED50.A101 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[6, 1] * 0.5), clevel = "A101", type = "absolute", interval = "delta")

ED50.A103 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[7, 1] * 0.5), clevel = "A103", type = "absolute", interval = "delta")

ED50.A75 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[8, 1] * 0.5), clevel = "A75", type = "absolute", interval = "delta")
...
EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[6, 1] * 0.5),
                     (summary(model.LL.3)$coefficients[5, 1] * 0.5)), 
       compMatch = c("A101", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[7, 1] * 0.5),
                     (summary(model.LL.3)$coefficients[5, 1] * 0.5)), 
       compMatch = c("A103", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[8, 1] * 0.5),
                     (summary(model.LL.3)$coefficients[5, 1] * 0.5)), 
...
compParm(model.LL.3, strVal = "e", operator = "-") # Remember that for assymetric models, the paramenter"e" does not reflect the ED50. 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Atrazine/2023/Atrazine_Absolute ED/MIs_Atra Only_GH_All Pops_ DR Percent Bio.tiff",
...
points(x = c(ED50.A82[1, 1], 
             ...
             ED50.A103[1, 1],
             ED50.A75[1, 1]),
       y = c((summary(model.LL.3)$coefficients[5, 1]/2), 
             (summary(model.LL.3)$coefficients[6, 1]/2),
             (summary(model.LL.3)$coefficients[7, 1]/2),
             (summary(model.LL.3)$coefficients[8, 1]/2)),
       pch = 8,
       cex = 1.3)
