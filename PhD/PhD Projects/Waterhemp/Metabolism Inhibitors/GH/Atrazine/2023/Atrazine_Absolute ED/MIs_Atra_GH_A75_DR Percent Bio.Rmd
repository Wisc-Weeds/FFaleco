...
summary.atra <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A75, herb=="atra"),
          FUN=fun)


summary.atra_GST <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A75, herb=="atra+GST"),
          FUN=fun)


summary.atra_P450 <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A75, herb=="atra+P450"),
          FUN=fun)
...
model.LL2.3 <- drm(percent_bio ~ rate, herb, fct = LL2.3(), data = data_A75, type = "continuous")
...
anova(model.LL2.3, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model W1.3
...
ED50.atra <- ED(model.LL2.3, (summary(model.LL2.3)$coefficients[4, 1] * 0.5), clevel = "atra", type = "absolute", interval = "delta")

ED50.atra.P450 <- ED(model.LL2.3, (summary(model.LL2.3)$coefficients[5, 1] * 0.5), clevel = "atra+P450", type = "absolute", interval = "delta")

ED50.atra.GST <- ED(model.LL2.3, (summary(model.LL2.3)$coefficients[6, 1] * 0.5), clevel = "atra+GST", type = "absolute", interval = "delta")
...
EDcomp(model.LL2.3, c((summary(model.LL2.3)$coefficients[4, 1] * 0.5),
                     (summary(model.LL2.3)$coefficients[6, 1] * 0.5)), 
