---
title: "Waterhemp GH Ametryne Percent Bio"
...
data <- read.csv("Data Sheet_Ametryne_GH_2025.csv")
...
summary <- summaryBy(cbind(percent_bio) ~ pop + rate, data = data,
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, pop, fct = LL.4(), data = data, type = "continuous")
...
model.W1.2 <- drm(percent_bio ~ rate, pop, fct = W1.2(), data = data, type = "continuous")
...
ED50.A106 <- ED(model.W1.2, 0.5, clevel = "A106", type = "absolute", interval = "delta")
...
EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A101", "A106"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A103", "A106"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A75", "A106"), type = "absolute", interval = "none", reverse = F)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Ametryne/Ame_GH_All Pops_DR Percent Bio.tiff",
...
plot(model.W1.2,
     type = "average",
     xlab = expression(paste("Ametryne rate (g ai ha"^-1, ")")),
