---
title: "Waterhemp MIs GH Fomesafen Percent Bio"
...
data <- read.csv("Data Sheet_Fomesafen_GH.csv")

str(data)


data_fome <- data %>% filter(herb == "fome")
...
summary.fome <- summaryBy(cbind(percent_bio) ~ pop + rate, data = subset(data_fome),
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, pop, fct = LL.4(), data = data_fome, type = "continuous")
...
model.W1.3 <- drm(percent_bio ~ rate, pop, fct = W1.3(),  data = data_fome, type = "continuous")
...
ED50.A82 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[5, 1] * 0.5), clevel = "A82", type = "absolute", interval = "delta")

ED50.A101 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[6, 1] * 0.5), clevel = "A101", type = "absolute", interval = "delta")

ED50.A103 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[7, 1] * 0.5), clevel = "A103", type = "absolute", interval = "delta")

ED50.A75 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[8, 1] * 0.5), clevel = "A75", type = "absolute", interval = "delta")
...
EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[6, 1] * 0.5),
                     (summary(model.W1.3)$coefficients[5, 1] * 0.5)), 
       compMatch = c("A101", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[7, 1] * 0.5),
                     (summary(model.W1.3)$coefficients[5, 1] * 0.5)), 
       compMatch = c("A103", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[8, 1] * 0.5),
                     (summary(model.W1.3)$coefficients[5, 1] * 0.5)), 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Fomesafen/Fomesafen_Absolute ED/MIs_Fome Only_GH_All Pops_ DR Percent Bio.tiff",
