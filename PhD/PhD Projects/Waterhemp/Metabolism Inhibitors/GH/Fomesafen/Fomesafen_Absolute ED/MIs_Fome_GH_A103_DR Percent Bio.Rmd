...
summary.fome <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A103, herb=="fome"),
          FUN=fun)


summary.fome_GST <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A103, herb=="fome+GST"),
          FUN=fun)


summary.fome_P450 <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A103, herb=="fome+P450"),
          FUN=fun)
...
model.W1.3 <- drm(percent_bio ~ rate, herb, fct = W1.3(), data = data_A103, type = "continuous")
...
ED50.fome <- ED(model.W1.3, (summary(model.W1.3)$coefficients[4, 1] * 0.5), clevel = "fome", type = "absolute", interval = "delta")

ED50.fome.P450 <- ED(model.W1.3, (summary(model.W1.3)$coefficients[5, 1] * 0.5), clevel = "fome+P450", type = "absolute", interval = "delta")

ED50.fome.GST <- ED(model.W1.3, (summary(model.W1.3)$coefficients[6, 1] * 0.5), clevel = "fome+GST", type = "absolute", interval = "delta")
...
EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[4, 1] * 0.5),
       ...
       compMatch = c("fome", "fome+P450"), type = "absolute", interval = "none", reverse = T)


EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[4, 1] * 0.5),
                     (summary(model.W1.3)$coefficients[6, 1] * 0.5)), 
       compMatch = c("fome", "fome+GST"), type = "absolute", interval = "none", reverse = T)
...
#tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Fomesafen/Fomesafen_Absolute ED/MIs_Fome_GH_A101_DR Percent Bio.tiff",
