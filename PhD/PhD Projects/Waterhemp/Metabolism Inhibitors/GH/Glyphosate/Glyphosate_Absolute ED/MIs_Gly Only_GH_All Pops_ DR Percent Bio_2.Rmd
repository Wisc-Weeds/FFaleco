...
model.W2.3 <- drm(percent_bio ~ rate, pop, fct = W2.3(),  data = data_gly, type = "continuous")
...
ED50.A82 <- ED(model.W2.3, (summary(model.W2.3)$coefficients[5, 1] * 0.5), clevel = "A82", type = "absolute", interval = "delta")

ED50.A101 <- ED(model.W2.3, (summary(model.W2.3)$coefficients[6, 1] * 0.5), clevel = "A101", type = "absolute", interval = "delta")

ED50.A103 <- ED(model.W2.3, (summary(model.W2.3)$coefficients[7, 1] * 0.5), clevel = "A103", type = "absolute", interval = "delta")

ED50.A75 <- ED(model.W2.3, (summary(model.W2.3)$coefficients[8, 1] * 0.5), clevel = "A75", type = "absolute", interval = "delta")
...
EDcomp(model.W2.3, c((summary(model.W2.3)$coefficients[6, 1] * 0.5),
                     (summary(model.W2.3)$coefficients[5, 1] * 0.5)), 
       compMatch = c("A101", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W2.3, c((summary(model.W2.3)$coefficients[7, 1] * 0.5),
                     (summary(model.W2.3)$coefficients[5, 1] * 0.5)), 
       compMatch = c("A103", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W2.3, c((summary(model.W2.3)$coefficients[8, 1] * 0.5),
                     (summary(model.W2.3)$coefficients[5, 1] * 0.5)), 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Glyphosate/Glyphosate_Absolute ED/MIs_Gly Only_GH_All Pops_ DR Percent Bio_2.tiff",
...
points(x = c(ED50.A82[1, 1], 
             ...
             ED50.A103[1, 1],
             ED50.A75[1, 1]),
       y = c((summary(model.W2.3)$coefficients[5, 1]/2), 
             (summary(model.W2.3)$coefficients[6, 1]/2),
             (summary(model.W2.3)$coefficients[7, 1]/2),
             (summary(model.W2.3)$coefficients[8, 1]/2)),
       pch = 8,
       cex = 1.3)
