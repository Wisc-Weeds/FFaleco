...
summary.gly <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A82, herb=="gly"),
          FUN=fun)


summary.gly_GST <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A82, herb=="gly+GST"),
          FUN=fun)


summary.gly_P450 <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A82, herb=="gly+P450"),
          FUN=fun)
...
model.LL.3 <- drm(biomass ~ rate, herb, fct = LL.3(), data = data_A82, type = "continuous")
...
ED50.gly <- ED(model.LL.3, (summary(model.LL.3)$coefficients[4, 1] * 0.5), clevel = "gly", type = "absolute", interval = "none")

ED50.gly.P450 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[5, 1] * 0.5), clevel = "gly+P450", type = "absolute", interval = "none")

ED50.gly.GST <- ED(model.LL.3, (summary(model.LL.3)$coefficients[6, 1] * 0.5), clevel = "gly+GST", type = "absolute", interval = "none")


# interval = "none" because it is a symmetric model. For assymetric should use "delta"
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Glyphosate/Glyphosate_Absolute ED/MIs_Gly_GH_A82_DR Biomass.tiff",
