...
summary.gly <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A101, herb=="gly"),
          FUN=fun)


summary.gly_GST <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A101, herb=="gly+GST"),
          FUN=fun)


summary.gly_P450 <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A101, herb=="gly+P450"),
          FUN=fun)
...
model.LL.3 <- drm(percent_bio ~ rate, herb, fct = LL.3(), data = data_A101, type = "continuous")
...
anova(model.LL.3, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model W1.3
...
ED50.gly <- ED(model.LL.3, (summary(model.LL.3)$coefficients[4, 1] * 0.5), clevel = "gly", type = "absolute", interval = "delta")

ED50.gly.P450 <- ED(model.LL.3, (summary(model.LL.3)$coefficients[5, 1] * 0.5), clevel = "gly+P450", type = "absolute", interval = "delta")

ED50.gly.GST <- ED(model.LL.3, (summary(model.LL.3)$coefficients[6, 1] * 0.5), clevel = "gly+GST", type = "absolute", interval = "delta")


# interval = "none" because it is a symmetric model. For asymmetric should use "delta"
...
EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[4, 1] * 0.5),
       ...
       compMatch = c("gly", "gly+P450"), type = "absolute", interval = "none", reverse = T)


EDcomp(model.LL.3, c((summary(model.LL.3)$coefficients[4, 1] * 0.5),
                     (summary(model.LL.3)$coefficients[6, 1] * 0.5)), 
       compMatch = c("gly", "gly+GST"), type = "absolute", interval = "none", reverse = T)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Glyphosate/Glyphosate_Absolute ED/MIs_Gly_GH_A101_DR Percent Bio.tiff",
...
points(x = c(ED50.gly[1, 1],
             ED50.gly.P450[1, 1],
             ED50.gly.GST[1, 1]),
       y = c((summary(model.LL.3)$coefficients[4, 1]/2), 
             (summary(model.LL.3)$coefficients[5, 1]/2),
