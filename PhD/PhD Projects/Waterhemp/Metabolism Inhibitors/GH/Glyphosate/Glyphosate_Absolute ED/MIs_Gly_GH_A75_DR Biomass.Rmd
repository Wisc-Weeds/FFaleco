...
summary.gly <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A75, herb=="gly"),
          FUN=fun)


summary.gly_GST <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A75, herb=="gly+GST"),
          FUN=fun)


summary.gly_P450 <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A75, herb=="gly+P450"),
          FUN=fun)
...
ED50.gly <- ED(model.LL4, (summary(model.LL4)$coefficients[7, 1] * 0.5), clevel = "gly", type = "absolute", interval = "none")

ED50.gly.P450 <- ED(model.LL4, (summary(model.LL4)$coefficients[8, 1] * 0.5), clevel = "gly+P450", type = "absolute", interval = "none")

ED50.gly.GST <- ED(model.LL4, (summary(model.LL4)$coefficients[9, 1] * 0.5), clevel = "gly+GST", type = "absolute", interval = "none")
...
EDcomp(model.LL4, c((summary(model.LL4)$coefficients[7, 1] * 0.5),
                     (summary(model.LL4)$coefficients[8, 1] * 0.5)), 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Glyphosate/Glyphosate_Absolute ED/MIs_Gly_GH_A75_DR Biomass.tiff",
...
points(x = c(ED50.gly[1, 1],
             ED50.gly.P450[1, 1],
             ED50.gly.GST[1, 1]),
       y = c((summary(model.LL4)$coefficients[7, 1]/2), 
             (summary(model.LL4)$coefficients[8, 1]/2),
             (summary(model.LL4)$coefficients[9, 1]/2)),
       pch = 8,
       cex = 1.3)
