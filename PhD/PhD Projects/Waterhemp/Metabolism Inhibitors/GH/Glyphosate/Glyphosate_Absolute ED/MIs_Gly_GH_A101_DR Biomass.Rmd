...
summary.gly <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A101, herb=="gly"),
          FUN=fun)


summary.gly_GST <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A101, herb=="gly+GST"),
          FUN=fun)


summary.gly_P450 <- summaryBy(cbind(biomass) ~ herb + rate, data = subset(data_A101, herb=="gly+P450"),
          FUN=fun)
...
model.W2.3 <- drm(biomass ~ rate, herb, fct = W2.3(), data = data_A101, type = "continuous")
...
anova(model.W2.3, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model W1.3
...
ED50.gly <- ED(model.W2.3, (summary(model.W2.3)$coefficients[4, 1] * 0.5), clevel = "gly", type = "absolute", interval = "delta")

ED50.gly.P450 <- ED(model.W2.3, (summary(model.W2.3)$coefficients[5, 1] * 0.5), clevel = "gly+P450", type = "absolute", interval = "delta")

ED50.gly.GST <- ED(model.W2.3, (summary(model.W2.3)$coefficients[6, 1] * 0.5), clevel = "gly+GST", type = "absolute", interval = "delta")
...
EDcomp(model.W2.3, c((summary(model.W2.3)$coefficients[4, 1] * 0.5),
                     (summary(model.W2.3)$coefficients[6, 1] * 0.5)), 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Glyphosate/Glyphosate_Absolute ED/MIs_Gly_GH_A101_DR Biomass.tiff",
...
points(x = c(ED50.gly[1, 1],
             ED50.gly.P450[1, 1],
             ED50.gly.GST[1, 1]),
       y = c((summary(model.W2.3)$coefficients[4, 1]/2), 
             (summary(model.W2.3)$coefficients[5, 1]/2),
