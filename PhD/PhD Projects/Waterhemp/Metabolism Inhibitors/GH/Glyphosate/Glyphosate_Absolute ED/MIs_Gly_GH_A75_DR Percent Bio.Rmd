...
summary.gly <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A75, herb=="gly"),
          FUN=fun)


summary.gly_GST <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A75, herb=="gly+GST"),
          FUN=fun)


summary.gly_P450 <- summaryBy(cbind(percent_bio) ~ herb + rate, data = subset(data_A75, herb=="gly+P450"),
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, herb, fct = LL.4(), data = data_A75, type = "continuous")
...
model.LL.3u <- drm(percent_bio ~ rate, herb, fct = LL.3u(), data = data_A75, type = "continuous")

plot(fitted(model.LL.3u), residuals(model.LL.3u))

qqnorm(residuals(model.LL.3u))


modelFit(model.LL.3u)

summary(model.LL.3u)


anova(model.LL.3u, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model W1.3
...
ED50.gly <- ED(model.LL.3u, 0.5, clevel = "gly", type = "absolute", interval = "delta")

ED50.gly.P450 <- ED(model.LL.3u, 0.5, clevel = "gly+P450", type = "absolute", interval = "delta")

ED50.gly.GST <- ED(model.LL.3u, 0.5, clevel = "gly+GST", type = "absolute", interval = "delta")
...
compParm(model.LL.3u, strVal = "b", operator = "-")


# Comparing d parameter (upper limit)
compParm(model.LL.3u, strVal = "d", operator = "-")


# Comparing e parameter (Infletion Point)
compParm(model.LL.3u, strVal = "e", operator = "-") # Remember that for assymetric models, the paramenter"e" does not reflect the ED50. 
