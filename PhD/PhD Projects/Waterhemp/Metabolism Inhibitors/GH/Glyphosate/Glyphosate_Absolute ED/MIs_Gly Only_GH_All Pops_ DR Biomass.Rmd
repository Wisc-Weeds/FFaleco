---
title: "Waterhemp MIs GH Glyphosate Biomass"
...
summary.gly <- summaryBy(cbind(biomass) ~ pop + rate, data = subset(data_gly),
          FUN=fun)
...
model.LL4 <- drm(biomass ~ rate, pop, fct = LL.4(), data = data_gly, type = "continuous")
...
model.W2.4 <- drm(biomass ~ rate, pop, fct = W2.4(),  data = data_gly, type = "continuous")

plot(fitted(model.W2.4), residuals(model.W2.4))

qqnorm(residuals(model.W2.4))
...
anova(model.W2.4, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model LL.3
...
ED50.A82 <- ED(model.W2.4, (summary(model.W2.4)$coefficients[9, 1] * 0.5), clevel = "A82", type = "absolute", interval = "delta")

ED50.A101 <- ED(model.W2.4, (summary(model.W2.4)$coefficients[10, 1] * 0.5), clevel = "A101", type = "absolute", interval = "delta")

ED50.A103 <- ED(model.W2.4, (summary(model.W2.4)$coefficients[11, 1] * 0.5), clevel = "A103", type = "absolute", interval = "delta")

ED50.A75 <- ED(model.W2.4, (summary(model.W2.4)$coefficients[12, 1] * 0.5), clevel = "A75", type = "absolute", interval = "delta")
...
EDcomp(model.W2.4, c((summary(model.W2.4)$coefficients[10, 1] * 0.5),
                     (summary(model.W2.4)$coefficients[9, 1] * 0.5)), 
       compMatch = c("A101", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W2.4, c((summary(model.W2.4)$coefficients[11, 1] * 0.5),
                     (summary(model.W2.4)$coefficients[9, 1] * 0.5)), 
       compMatch = c("A103", "A82"), type = "absolute", interval = "none", reverse = F)


EDcomp(model.W2.4, c((summary(model.W2.4)$coefficients[12, 1] * 0.5),
                     (summary(model.W2.4)$coefficients[9, 1] * 0.5)), 
...
compParm(model.W2.4, strVal = "b", operator = "-")


# Comparing d parameter (upper limit)
compParm(model.W2.4, strVal = "d", operator = "-")


# Comparing e parameter (Infletion Point)
compParm(model.W2.4, strVal = "e", operator = "-") # Remember that for assymetric models, the paramenter"e" does not reflect the ED50. 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GH/Glyphosate/Glyphosate_Absolute ED/MIs_Gly Only_GH_All Pops_ DR Biomass.tiff",
...
points(x = c(ED50.A82[1, 1], 
             ...
             ED50.A103[1, 1],
             ED50.A75[1, 1]),
       y = c((summary(model.W2.4)$coefficients[9, 1]/2), 
             (summary(model.W2.4)$coefficients[10, 1]/2),
             (summary(model.W2.4)$coefficients[11, 1]/2),
             (summary(model.W2.4)$coefficients[12, 1]/2)),
       pch = 8,
       cex = 1.3)
