...
data.fome.MIs.ambient <- data %>% filter(pop == "A101", temp == "ambient")
...
summary <- summaryBy(cbind(percent_bio) ~ treat + rate, data = data.fome.MIs.ambient,
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, treat, fct = LL.4(), data = data.fome.MIs.ambient, type = "continuous")
...
model.W1.2 <- drm(percent_bio ~ rate, treat, fct = W1.2(), data = data.fome.MIs.ambient, type = "continuous")
...
EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A101+fome+P450+ambient", "A101+fome+ambient"), type = "absolute", interval = "none", reverse = T)



EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A101+fome+GST+ambient", "A101+fome+ambient"), type = "absolute", interval = "none", reverse = T)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Fomesafen/Fomesafen_Absolute ED_Multiple Comparisons/MIs_Fome + MIs_Ambient_GC_ DR Percent Bio.tiff",
...
points(x = c(ED50.A101.fome.ambient [1, 1],
