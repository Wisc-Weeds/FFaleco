...
data.meso <- data %>% filter(herb == "meso")
...
summary <- summaryBy(cbind(percent_bio) ~ treat + rate, data = data.meso,
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, treat, fct = LL.4(), data = data.meso, type = "continuous")
...
model.W1.2 <- drm(percent_bio ~ rate, treat, fct = W1.2(), data = data.meso, type = "continuous")
...
ED50.A82.meso.increased <- ED(model.W1.2, 0.5, clevel = "A82+meso+increased", type = "absolute", interval = "delta")

ED50.A82.meso.ambient <- ED(model.W1.2, 0.5, clevel = "A82+meso+ambient", type = "absolute", interval = "delta")



ED50.A101.meso.increased <- ED(model.W1.2, 0.5, clevel = "A101+meso+increased", type = "absolute", interval = "delta")
...
EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A82+meso+increased", "A82+meso+ambient"), type = "absolute", interval = "none", reverse = T)



EDcomp(model.W1.2, c(0.5, 0.5), 
       compMatch = c("A101+meso+increased", "A101+meso+ambient"), type = "absolute", interval = "none", reverse = T)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/2023/Mesotrione_Absolute ED_Multiple Comparisons/MIs_Meso Only_GC_ DR Percent Bio.tiff",
...
points(x = c(ED50.A82.meso.increased[1, 1],
             ED50.A82.meso.ambient [1, 1],
             ED50.A101.meso.increased [1, 1],
