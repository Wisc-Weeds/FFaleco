...
data.A101.meso.GST <- data %>% filter(pop == "A101", herb == "meso+GST")
...
summary.A101.meso.GST <- summaryBy(cbind(percent_bio) ~ pop + rate + temp + herb, data = subset(data.A101.meso.GST),
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, temp, fct = LL.4(), data = data.A101.meso.GST, type = "continuous")
...
model.W1.2 <- drm(percent_bio ~ rate, temp, fct = W1.2(), data = data.A101.meso.GST, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/2023/Mesotrione_Absolute ED/MIs_Mesotrione +GST_GC_A101_ DR Percent Bio.tiff",
...
plot(model.W1.2,
     type = "average",
     xlab = expression(paste("Mesotrione + GST rate (g ai ha"^-1, ")")),
