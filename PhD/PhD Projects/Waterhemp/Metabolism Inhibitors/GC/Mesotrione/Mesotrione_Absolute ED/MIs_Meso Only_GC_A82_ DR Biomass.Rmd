...
data.A82.meso <- data %>% filter(pop == "A82", herb == "meso")
...
summary.A82.meso <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data.A82.meso),
          FUN=fun)
...
model.LL4 <- drm(biomass ~ rate, temp, fct = LL.4(), data = data.A82.meso, type = "continuous")
...
model.LL2.3 <- drm(biomass ~ rate, temp, fct = LL2.3(), data = data.A82.meso, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/Mesotrione_Absolute ED/MIs_Meso Only_GC_A82_ DR Biomass.tiff",
