...
data_Brook_high <- data %>% filter(pop == "Brook", temp == "H")
...
summary.Brook_high <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data_Brook_high),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, herb, fct = LL.4(), data = data_Brook_high, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, herb, fct = LL.3(),  data = data_Brook_high, type = "continuous")
...
data_Brook_low <- data %>% filter(pop == "Brook", temp == "L")
...
summary.Brook_low <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data_Brook_low),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, herb, fct = LL.4(), data = data_Brook_low, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, herb, fct = LL.3(),  data = data_Brook_low, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/Meso_Brook_Low_DR_biomass.tiff",
     width = 10, height = 8, units = "in", res = 600, compression = "lzw")
...
text(x = 265, y = 2.0, 
     labels = expression(paste("ED"[50] ~ "meso = 10.6 ("%+-% "1.7)")),
     cex = 1.1,
     col = "black",
     font = 2)
...
text(x = 330, y = 1.8, 
     labels = expression(paste("ED"[50] ~ "meso+P450 = 0.03 ("%+-% "0.1)")),
     cex = 1.1,
     col = "black",
     font = 2)
