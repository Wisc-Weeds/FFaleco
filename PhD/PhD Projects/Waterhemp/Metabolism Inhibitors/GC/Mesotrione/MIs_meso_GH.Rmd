...
# CONFIRMING RESISTANCE AT HIGH TEMP
...
data_meso_high <- data %>% filter(herb == "meso", temp == "H")
...
summary.meso_high <- summaryBy(cbind(biomass) ~ pop + rate + temp, data = subset(data_meso_high),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, pop, fct = LL.4(), data = data_meso_high, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, pop, fct = LL.3(),  data = data_meso_high, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/Meso_High_DR_biomass.tiff",
...
text(x = 300, y = 2.2, 
     labels = expression(paste("ED"[50] ~ "Brook = 13.0 ("%+-% "1.2)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 270, y = 1.95, 
     labels = expression(paste("ED"[50] ~ "A82 = 0.9 ("%+-% "1.1)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
# CONFIRMING RESISTANCE AT LOW TEMP
...
data_meso_low <- data %>% filter(herb == "meso", temp == "L")
...
summary.meso_low<- summaryBy(cbind(biomass) ~ pop + rate + temp, data = subset(data_meso_low),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, pop, fct = LL.4(), data = data_meso_low, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, pop, fct = LL.3(),  data = data_meso_low, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/Meso_Low_DR_biomass.tiff",
     width = 10, height = 8, units = "in", res = 600, compression = "lzw")
...
text(x = 300, y = 1.8, 
     labels = expression(paste("ED"[50] ~ "Brook = 10.6 ("%+-% "1.4)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 270, y = 1.6, 
     labels = expression(paste("ED"[50] ~ "A82 = 7.0 ("%+-% "3.2)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 200, y = 1.4, 
     labels = expression("p-value < 0.29"),
     cex = 1.2,
     col = "black",
     font = 2)
