...
data.A82.meso.P450 <- data %>% filter(pop == "A82", herb == "meso+P450")
...
summary.A82.meso.P450 <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data.A82.meso.P450),
          FUN=fun)
...
model.LL2.3 <- drm(biomass ~ rate, temp, fct = LL2.3(), data = data.A82.meso.P450, type = "continuous")


plot(fitted(model.LL2.3), residuals(model.LL2.3))

                
modelFit(model.LL2.3) # higher the p-value, better.
...
mselect(model.LL2.3,  list(L.3(), L.4(), L.5(), LL.2(), LL.3(), LL.3u(), LL.4(), LL.5(), W1.2(), W1.3(), W1.4(), W2.2(), W2.3(), W2.4(), BC.4(), BC.5(), LL2.2(), LL2.3(), LL2.3u(), LL2.4(), LL2.5(), AR.2(), AR.3(), MM.2(), MM.3(), baro5()), linreg = T)
...
EDcomp(model.LL2.3, c((summary(model.LL2.3)$coefficients[4, 1] * 0.5),
       ...
       compMatch = c("elevated", "ambient"), type = "absolute", interval = "none", reverse = T, logBase = exp(1))
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/Mesotrione_Absolute ED/MIs_Meso+P450_GC_A82_DR Biomass.tiff",
...
plot(model.LL2.3,
     type = "average",
     xlab = expression(paste("Mesotrione+P450 rate (g ai ha"^-1, ")")),
