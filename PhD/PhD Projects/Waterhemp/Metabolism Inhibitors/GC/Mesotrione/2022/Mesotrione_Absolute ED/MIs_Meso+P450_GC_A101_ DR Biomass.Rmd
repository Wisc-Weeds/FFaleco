...
data.A101.meso.P450 <- data %>% filter(pop == "A101", herb == "meso+P450")
...
summary.A101.meso.P450 <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data.A101.meso.P450),
          FUN=fun)
...
model.LL.4 <- drm(biomass ~ rate, temp, fct = LL.4(), data = data.A101.meso.P450, type = "continuous")
...
model.LL2.3 <- drm(biomass ~ rate, temp, fct = LL2.3(), data = data.A101.meso.P450, type = "continuous")
...
EDcomp(model.LL2.3, c((summary(model.LL2.3)$coefficients[4, 1] * 0.5),
       ...
       compMatch = c("elevated", "ambient"), type = "absolute", interval = "none", reverse = F, logBase = exp(1))
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Mesotrione/Mesotrione_Absolute ED/MIs_Meso+P450_GC_A101_DR Biomass.tiff",
