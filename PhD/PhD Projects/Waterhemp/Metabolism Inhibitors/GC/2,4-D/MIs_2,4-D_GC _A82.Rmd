...
data_A82_24D <- data %>% filter(pop == "A82", herb == "2,4D")
...
summary.A82_24D  <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data_A82_24D),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, temp, fct = LL.4(), data = data_A82_24D, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, temp, fct = LL.3(),  data = data_A82_24D, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/2,4-D/A82_2,4-D_DR_biomass.tiff",
...
text(x = 3500, y = 1.5, 
     labels = expression(paste("ED"[50] ~ "elevated = 126.0 ("%+-% "6.7)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 3450, y = 1.3, 
     labels = expression(paste("ED"[50] ~ "ambient = 44.1 ("%+-% "34.8)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 2000, y = 1.1, 
     labels = expression("p-value = 0.02"),
     cex = 1.2,
     col = "black",
     font = 2)
...
data_A82_24D_P450 <- data %>% filter(pop == "A82", herb == "2,4D+P450")
...
summary.A82_24D_P450 <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data_A82_24D_P450),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, temp, fct = LL.4(), data = data_A82_24D_P450, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, temp, fct = LL.3(),  data = data_A82_24D_P450, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/2,4-D/A82_2,4-D_P450_DR_biomass.tiff",
     width = 10, height = 8, units = "in", res = 600, compression = "lzw")
...
plot(model.LL3,
     type = "average",
     xlab = expression(paste("2,4-D+P450 dose (g ai ha"^-1, ")")),
     ylab = expression(paste("A82 biomass (g plant"^-1, ")")),
     ylim = c(0, 4),
     col = c("darkgreen", "red"),
     pch = c(19, 17),
...
text(x = 3500, y = 1.5, 
     labels = expression(paste("ED"[50] ~ "ambient = 109.9 ("%+-% "14.6)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 3450, y = 1.3, 
     labels = expression(paste("ED"[50] ~ "elevated = 22.9 ("%+-% "17.6)")),
     cex = 1.2,
     col = "black",
     font = 2)
...
text(x = 2000, y = 1.1, 
     labels = expression("p-value < 0.001"),
     cex = 1.2,
     col = "black",
     font = 2)
