...
data_A101_24D <- data %>% filter(pop == "A101", herb == "2,4D")
...
summary.A101_24D <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data_A101_24D),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, temp, fct = LL.4(), data = data_A101_24D, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, temp, fct = LL.3(),  data = data_A101_24D, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/2,4-D/GC_2,4-D Only_A101_DR biomass.tiff",
...
text(x = 3300, y = 2.8, 
     labels = expression("Elevated Upper Limit = 3.2 ("%+-% "0.2)" ^A),
     cex = 1.0,
     col = "black",
     font = 2)
...
text(x = 3300, y = 2.6, 
     labels = expression("Ambient Upper Limit = 2.0 ("%+-% "0.2)" ^B),
     cex = 1.0,
     col = "black",
     font = 2)
...
data_A101_24D_P450 <- data %>% filter(pop == "A101", herb == "2,4D+P450")
...
summary.Brook_24D_P450 <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data_A101_24D_P450),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, temp, fct = LL.4(), data = data_A101_24D_P450, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, temp, fct = LL.3(),  data = data_A101_24D_P450, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/2,4-D/GC_2,4-D+P450_A101_DR biomass.tiff",
     width = 10, height = 8, units = "in", res = 600, compression = "lzw")
...
plot(model.LL3,
     type = "average",
     xlab = expression(paste("2,4-D+P450-inhibitor dose (g ae ha"^-1, ")")),
     ylab = expression(paste("A101 biomass (g plant"^-1, ")")),
     ylim = c(0, 5),
     col = c("darkgreen", "red"),
     pch = c(19, 17),
...
text(x = 3300, y = 2.8, 
     labels = expression("Elevated Upper Limit = 3.1 ("%+-% "0.2)" ^A),
     cex = 1.0,
     col = "black",
     font = 2)
...
text(x = 3300, y = 2.6, 
     labels = expression("Ambient Upper Limit = 2.2 ("%+-% "0.2)" ^B),
     cex = 1.0,
     col = "black",
     font = 2)
