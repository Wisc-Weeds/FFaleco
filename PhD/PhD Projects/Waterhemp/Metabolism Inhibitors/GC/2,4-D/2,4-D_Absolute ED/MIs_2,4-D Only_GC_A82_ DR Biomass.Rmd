...
data.A82.24D <- data %>% filter(pop == "A82", herb == "2,4D")
...
summary.A82.24D <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data.A82.24D),
          FUN=fun)
...
model.LL4 <- drm(biomass ~ rate, temp, fct = LL.4(), data = data.A82.24D, type = "continuous")
...
EDcomp(model.LL2.3, c((summary(model.LL2.3)$coefficients[4, 1] * 0.5),
                     (summary(model.LL2.3)$coefficients[3, 1] * 0.5)), 
       compMatch = c("elevated", "ambient"), type = "absolute", interval = "fls", reverse = F, logBase = exp(1))
...
comped <- comped(c(ED50.ambient[1, 1], ED50.elevated[1, 1]), c(ED50.ambient[1, 2], ED50.elevated[1, 2]), log = F, operator = "/", level = 0.95)



EDcomp(ED50.elevated[1, 1], ED50.ambient[1, 1], type = "absolute")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/2,4-D/2,4-D_Absolute ED/MIs_2,4-D Only_GC_A82_ DR Biomass.tiff",
