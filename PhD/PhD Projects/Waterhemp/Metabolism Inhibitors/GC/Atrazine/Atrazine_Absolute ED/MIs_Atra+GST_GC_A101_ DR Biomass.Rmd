...
data.A101.atra.GST <- data %>% filter(pop == "A101", herb == "atra+GST")
...
summary.A101.atra.GST <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data.A101.atra.GST),
          FUN=fun)
...
model.LL4 <- drm(biomass ~ rate, temp, fct = LL.4(), data = data.A101.atra.GST, type = "continuous")
...
model.W1.3 <- drm(biomass ~ rate, temp, fct = W1.3(), data = data.A101.atra.GST, type = "continuous")
...
ED50.elevated <- ED(model.W1.3, (summary(model.W1.3)$coefficients[3, 1] * 0.5), clevel = "elevated", type = "absolute", interval = "delta")

ED50.ambient <- ED(model.W1.3, (summary(model.W1.3)$coefficients[4, 1] * 0.5), clevel = "ambient", type = "absolute", interval = "delta")
...
EDcomp(model.W1.3, c((summary(model.W1.3)$coefficients[4, 1] * 0.5),
       ...
       compMatch = c("elevated", "ambient"), type = "absolute", interval = "none", reverse = F)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Atrazine/Atrazine_Absolute ED/MIs_Atra+GST_GC_A101_ DR Biomass.tiff",
...
plot(model.W1.3,
     type = "average",
     xlab = expression(paste("Atrazine+GST rate (g ai ha"^-1, ")")),
