...
data.A82.atra.GST <- data %>% filter(pop == "A82", herb == "atra+GST")
...
summary.A82.atra.GST <- summaryBy(cbind(biomass) ~ pop + rate + temp + herb, data = subset(data.A82.atra.GST),
          FUN=fun)
...
model.LL4 <- drm(biomass ~ rate, temp, fct = LL.4(), data = data.A82.atra.GST, type = "continuous")
...
model.W1.3 <- drm(biomass ~ rate, temp, fct = W1.3(), data = data.A82.atra.GST, type = "continuous")
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Atrazine/Atrazine_Absolute ED/MIs_Atra+GST_GC_A82_ DR Biomass.tiff",
