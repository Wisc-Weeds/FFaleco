...
data.atra.MIs.ambient <- data %>% filter(pop == "A101", temp == "ambient")
...
summary <- summaryBy(cbind(percent_bio) ~ treat + rate, data = data.atra.MIs.ambient,
          FUN=fun)
...
model.LL4 <- drm(percent_bio ~ rate, treat, fct = LL.4(), data = data.atra.MIs.ambient, type = "continuous")
...
model.W2.2 <- drm(percent_bio ~ rate, treat, fct = W2.2(), data = data.atra.MIs.ambient, type = "continuous")
...
ED50.A101.atra.P450.ambient <- ED(model.W2.2, 0.5, clevel = "A101+atra+P450+ambient", type = "absolute", interval = "delta")


ED50.A101.atra.GST.ambient <- ED(model.W2.2, 0.5, clevel = "A101+atra+GST+ambient", type = "absolute", interval = "delta")
...
EDcomp(model.W2.2, c(0.5, 0.5), 
       compMatch = c("A101+atra+P450+ambient", "A101+atra+ambient"), type = "absolute", interval = "none", reverse = T)



EDcomp(model.W2.2, c(0.5, 0.5), 
       compMatch = c("A101+atra+GST+ambient", "A101+atra+ambient"), type = "absolute", interval = "none", reverse = T)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/PhD Projects/Waterhemp/Metabolism Inhibitors/GC/Atrazine/2023/Atrazine_Absolute ED_Multiple Comparisons/MIs_Atra + MIs_Ambient_GC_ DR Percent Bio.tiff",
...
points(x = c(ED50.A101.atra.ambient [1, 1],
