---
title: "Exacto 2023-112 A92 DR Control 21 DAT"
...
model.MM.2 <- drm(ve_21dat ~ rate, herb, fct = MM.2(), data = data, type = "continuous")



plot(fitted(model.MM.2), residuals(model.MM.2))

qqnorm(residuals(model.MM.2))
...
anova(model.MM.2, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model LL.3
...
ED50.reflex <- ED(model.MM.2, (summary(model.MM.2)$coefficients[1, 1] * 0.5), clevel = "reflex", type = "absolute", interval = "delta")

ED50.EXT1490 <- ED(model.MM.2, (summary(model.MM.2)$coefficients[2, 1] * 0.5), clevel = "EXT1490", type = "absolute", interval = "delta")
...
EDcomp(model.MM.2, c((summary(model.MM.2)$coefficients[2, 1] * 0.5),
                     (summary(model.MM.2)$coefficients[1, 1] * 0.5)), 
       compMatch = c("reflex", "EXT1490"), type = "absolute", interval = "none", reverse = F)



# Comparing b parameter (slope)
compParm(model.MM.2, strVal = "b", operator = "-")


# Comparing d parameter (upper limit)
compParm(model.MM.2, strVal = "d", operator = "-")


# Comparing e parameter (Infletion Point)
compParm(model.MM.2, strVal = "e", operator = "-") # Remember that for assymetric models, the paramenter"e" does not reflect the ED50. 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/Exacto/2023/2023-112/A92/2023-112_A92_DR Control_21 DAT.tiff",
