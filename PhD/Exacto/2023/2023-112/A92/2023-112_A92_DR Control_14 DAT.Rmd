---
title: "Exacto 2023-112 A92 DR Control 14 DAT"
...
model.LL4 <- drm(ve_14dat ~ rate, herb, fct = LL.4(),  data = data, type = "continuous")
...
model.L.3 <- drm(ve_14dat ~ rate, herb, fct = L.3(), data = data, type = "continuous")



plot(fitted(model.L.3), residuals(model.L.3))

qqnorm(residuals(model.L.3))
...
anova(model.L.3, model.LL4) # p-value > 0.05 means that there is no difference between models. Therefore, we can use model LL.3
...
ED50.reflex <- ED(model.L.3, (summary(model.L.3)$coefficients[3, 1] * 0.5), clevel = "reflex", type = "absolute", interval = "delta")

ED50.EXT1490 <- ED(model.L.3, (summary(model.L.3)$coefficients[4, 1] * 0.5), clevel = "EXT1490", type = "absolute", interval = "delta")
...
EDcomp(model.L.3, c((summary(model.L.3)$coefficients[4, 1] * 0.5),
                     (summary(model.L.3)$coefficients[3, 1] * 0.5)), 
       compMatch = c("reflex", "EXT1490"), type = "absolute", interval = "none", reverse = F)



# Comparing b parameter (slope)
compParm(model.L.3, strVal = "b", operator = "-")


# Comparing d parameter (upper limit)
compParm(model.L.3, strVal = "d", operator = "-")


# Comparing e parameter (Infletion Point)
compParm(model.L.3, strVal = "e", operator = "-") # Remember that for assymetric models, the paramenter"e" does not reflect the ED50. 
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/Exacto/2023/2023-112/A92/2023-112_A92_DR Control_14 DAT.tiff",
...
plot(model.L.3,
     ...
     ylab = expression(paste("Control 14 DAT (%)")),
     ylim = c(0, 100),
     #broken = T,
     col = c("darkgreen", "red"),
     pch = c(19, 17),
