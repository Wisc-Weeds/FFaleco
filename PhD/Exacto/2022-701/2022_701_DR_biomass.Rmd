---
title: "Exacto 2022-701 Dose Response biomass"
...
data <- read.csv("2022_701Data.csv")
...
fun <- function(x){
  c( mean = mean(x, na.rm = TRUE), VAR = var(x, na.rm = TRUE), SD = sd(x, na.rm = TRUE), SE = std.error(x, na.rm = TRUE), n = length(x[!is.na(x)]))}


summary.Reflex_EXT1576 <- summaryBy(biomass ~ rate + name, data = subset(data, name=="Reflex+EXT1576"),
          FUN=fun)


summary.Reflex_TolexME8470 <- summaryBy(biomass ~ rate + name, data = subset(data, name=="Reflex+TolexME8470"),
          FUN=fun)
...
model.LL4<- drm(biomass ~ rate, name, fct = LL.4(),  data = data)
...
model.LL3 <- drm(biomass ~ rate, name, fct = LL.3(),  data = data, type = "continuous")
...
EDcomp(model.LL3, c(50, 50), type = "relative", interval = "none", reverse = FALSE)

EDcomp(model.LL3, c(90, 90), type = "relative", interval = "none", reverse = FALSE)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/Exacto/2022-701/2022_701_DR_biomass.tiff",
