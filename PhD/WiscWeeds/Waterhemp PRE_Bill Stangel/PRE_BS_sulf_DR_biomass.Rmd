---
title: "Waterhemp PRE BS Sulfentrazone DR Biomass"
...
data <- read.csv("data_PRE_BS_sulf.csv")

str(data)


data2 <- read.csv("data_PRE_BS_sulf2.csv")
...
model.LL4<- drm(biomass ~ rate, pop, fct = LL.4(), data = data, type = "continuous", control = drmc(constr = F, errorm = F, method = "Brent"))



model.LL4<- drm(count ~ rate, pop, fct = LL.4(), data = data2, type = "continuous")
...
model.LL3<- drm(biomass ~ rate, pop, fct = LL.3(),  data = data, type = "continuous")
...
plot(model.LL3,
     ...
     ylab = expression(paste("Dry aboveground biomass (g plant"^-1, ")")),
     col = c("red", "darkgreen"),
     pch = c(19, 17),
     cex = 1.2,
     cex.axis = 1,
...
text(x = 650, y = 1.2, 
     labels = expression(paste("ED"[50] ~ "Ratio = 1.4 ("%+-% "0.2)")),
     cex = 1,
     col = "black",
     font = 2)

text(x = 480, y = 1.1, 
     labels = expression("p-value = 0.07"),
     cex = 1,
     col = "black",
     font = 2)
...
tiff(file="C:/Users/felip/OneDrive - UW-Madison/Documents/GitHub/FFaleco/PhD/Exacto/2022-108/DR_biomass.tiff",
