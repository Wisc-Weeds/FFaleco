---
title: "RAW.BETA BIOMASS REDUCTION - WATERHEMP ALL POST 3X"
...
waterhemp <- read.csv("All POST_Reduction_3x.csv")
...
plot_RAW <- ggplot(summary, aes(x = bio_red*100, y = reorder(herb, bio_red))) + 
  geom_point(size = 2) + 
  geom_vline(xintercept = 90, size = 1, colour = "red") +
  geom_errorbar(aes(xmin = (bio_red*100) - (ci*100), xmax = (bio_red*100) + (ci*100), width = 0.2)) + 
  labs(title = "Waterhemp Biomass Reduction 3x - POST", x = "Biomass Reduction (%)", y = "Herbicide") + 
  theme_classic() + 
...
model <- glmmTMB(bio_red ~ herb + (1|run) + (1|pop), data = waterhemp, beta_family(link = "logit"))
...
plot_RAW + geom_text(aes(label = c("a", "a", "a", "a", "a", "b", "c", "a")), vjust = -1) +
  ggsave("All POST 3x_RAW.BETA Tukey HSD.jpeg", units="in", width=8, height=5, dpi=600)
