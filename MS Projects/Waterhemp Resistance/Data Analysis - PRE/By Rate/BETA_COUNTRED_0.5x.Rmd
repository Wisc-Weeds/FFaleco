---
title: "BETA COUNT REDUCTION - WATERHEMP ALL PRE 0.5X"
...
waterhemp <- read.csv("All PRE_Reduction_0.5x.csv")
...
CLD_emm_herb <- if(requireNamespace("multcomp")) {
    multcomp::cld(emm_herb$emmeans, alpha = 0.05, Letters=letters, adjust="tukey",type = "response", contr = "pairwise", reversed = TRUE)}
...
plot(emm_herb, adjust = "tukey", comparisons = TRUE) + xlim(0.6, 1) + 
  ...
  geom_text(aes(label = c("c", "b", "a", "ab", "a")), vjust = -1) +
  labs(title = "Waterhemp Count Reduction 0.5x - PRE", x = "Count Reduction (%)", y = "Herbicide") + 
  theme_classic() + 
  ...
  ggsave("All PRE 0.5x_BETA Tukey HSD.jpeg", units="in", width=8, height=5, dpi=600)
