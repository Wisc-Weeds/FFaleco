...
ggplot(palmer, aes(x = count, y = reorder(herb, order), color = biomass)) + 
  geom_jitter(aes(color = biomass), alpha = 0.8, width = 0.3) +
  facet_grid(run ~ rate, labeller = label_both) + 
  scale_color_gradient(low = "yellow", high = "red") +
  theme_dark() +
  scale_x_continuous(limits = c(0, 90)) + 
  labs(title = "Exploratory Analysis - Palmer PRE", x = "Count", y = "Herbicide", color = "Biomass (g)") + 
